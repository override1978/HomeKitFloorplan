...
final class FloorplanOverlayViewModel {
    ...
    enum PanelContent: Equatable {
        ...
        case span
        /// I processi ancora in corso, scelti dalla testata del nastro.
        case runningSpans
    }
    ...
    /// Apre il riepilogo dei processi ancora in corso.
    func showRunningSpansDetail() {
        panelContent = .runningSpans
        if !isPanelVisible {
            withAnimation(.spring(response: 0.38, dampingFraction: 0.88)) {
                isPanelVisible = true
            }
