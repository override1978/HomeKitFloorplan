...
struct SensorSampleGate {
    ...
    mutating func shouldWrite(
        ...
        let expired = now.timeIntervalSince(last.at) >= Self.maxGap
        ...
        if moved { stats.changed += 1 } else { stats.heartbeat += 1 }
    ...
    struct Stats {
        ...
        var written: Int { changed + heartbeat + firstEver }

        var summary: String {
            let ratio = sampled > 0 ? Double(written) / Double(sampled) * 100 : 0
            return String(format:
                "campionate %d · archiviate %d (%.0f%%) — variazione %d, battito %d, prima volta %d",
                sampled, written, ratio, changed, heartbeat, firstEver)
        }
    }
